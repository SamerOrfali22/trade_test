...
class StreamPager<T> implements Pager<T> {
  ...
  final Stream<Result<PaginationModel<T>>> Function(int pageKey) onPageData;
  ...
  void _consumePageRequest() => _consumerSubscription = _pageNumberController
      .flatMap(
        (pageKey) => onPageData(pageKey)
            ...
            .map((result) => Tuple2(pageKey, result)),
      )
      .listen(
        (tuple) => tuple.item2.whenOrNull(
          onError: (origin, error, st) => _updateErrorState(origin, error, st, tuple.item1),
          onData: (origin, data) => _updateDataState(origin, data, tuple.item1),
        ),
      );

  void _updateErrorState(ResultOrigin origin, Object error, StackTrace? st, int pageKey) {
    _updatePagerState(error: error);
    // TODO(abd): we must unsubscribe to all page stream that bigger than page.
  }
  ...
  void _updateDataState(ResultOrigin origin, PaginationModel<T> data, int pageKey) {
    ...
    if (metaData == null) {
      ...
      _pagesMetaData[pageKey] = _PageMetaData(startIndex: startIndex, endIndex: startIndex + data.data.length);
      _updatePagerState(
        error: null,
        nextPageKey: data.totalPages.isNullOrZero || data.totalPages < pageKey ? null : pageKey + 1,
        itemList: [
    ...
    // update itemList case
      ...
      // TODO(abd): check item less than old array
      _updatePagerState(
        error: null,
        itemList: currentState..itemList?.setRange(startIndex, endIndex, data.data),
      );
    }
  }

  // TODO(abd): we must protect write on controller with mutex
